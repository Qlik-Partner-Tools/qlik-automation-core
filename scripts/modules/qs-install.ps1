...
$qsVersionBoth = @("Qlik Sense April 2019", 
 ...
 $qsVersionEula = @("Qlik Sense June 2020", "Qlik Sense September 2020", "Qlik Sense November 2020", "Qlik Sense February 2021", "Qlik Sense May 2021" )
    ...
    If ($server.sense.persistence -eq "shared" -and $server.sense.central -eq $true -and $server.name -eq $ENV:computername )
    {
        ...
        elseIf (Test-Path "c:\shared-content\binaries\Qlik_Sense_setup.exe")
        {
            ...
            if ($qsVersionDashOnly -contains $qsVer.name)
                ...
                elseif ($qsVersions -notcontains $qsVer.name -and $qsVersionDashOnly -notcontains $qsVer.name -and $qsVersionBoth -notcontains $qsVer.name -and $qsVersionEula -notcontains $qsVer.name)
                {
